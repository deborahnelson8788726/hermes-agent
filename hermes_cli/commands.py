...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "Новая сессия (новый ID + чистая история)", "Session",
               aliases=("reset",)),
    CommandDef("clear", "Очистить экран и начать новую сессию", "Session",
               cli_only=True),
    CommandDef("redraw", "Принудительная перерисовка UI (восстановление при сбое терминала)", "Session",
               cli_only=True),
    CommandDef("history", "Показать историю разговора", "Session",
               cli_only=True),
    CommandDef("save", "Сохранить текущий разговор", "Session",
               cli_only=True),
    CommandDef("retry", "Повторить последнее сообщение (отправить агенту заново)", "Session"),
    CommandDef("undo", "Удалить последний обмен пользователь/ассистент", "Session"),
    CommandDef("title", "Задать название текущей сессии", "Session",
               args_hint="[name]"),
    CommandDef("branch", "Ответвить текущую сессию (исследовать другой путь)", "Session",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("compress", "Сжать контекст разговора вручную", "Session",
               args_hint="[focus topic]"),
    CommandDef("rollback", "Список или восстановление чекпоинтов файловой системы", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Создание/восстановление снапшотов конфига и состояния Hermes", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Завершить все фоновые процессы", "Session"),
    CommandDef("approve", "Одобрить ожидающую опасную команду", "Session",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "Отклонить ожидающую опасную команду", "Session",
               gateway_only=True),
    CommandDef("background", "Запустить промпт в фоне", "Session",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "Показать активных агентов и задачи", "Session",
               aliases=("tasks",)),
    CommandDef("queue", "Поставить промпт в очередь на следующий ход (не прерывает)", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "Вставить сообщение после следующего вызова инструмента не прерывая", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Установить долгосрочную цель — Hermes работает над ней до достижения", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("status", "Показать информацию о сессии", "Session"),
    CommandDef("profile", "Показать активный профиль и домашнюю директорию", "Info"),
    CommandDef("sethome", "Сделать этот чат home channel (для cron-доставки)", "Session",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "Возобновить ранее названную сессию", "Session",
               args_hint="[name]"),

    # Configuration
    CommandDef("config", "Показать текущую конфигурацию", "Configuration",
               cli_only=True),
    CommandDef("model", "Сменить модель для этой сессии", "Configuration",
               aliases=("provider",), args_hint="[model] [--provider name] [--global]"),
    CommandDef("gquota", "Показать использование квоты Google Gemini Code Assist", "Info",
               cli_only=True),

    CommandDef("personality", "Установить готовый стиль общения", "Configuration",
               args_hint="[name]"),
    CommandDef("statusbar", "Переключить status bar с контекстом и моделью", "Configuration",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "Циклически переключать отображение прогресса инструментов: off → new → all → verbose",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "Включить/выключить runtime-метаданные в финальных ответах",
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "Переключить YOLO-режим (без подтверждений опасных команд)",
               "Configuration"),
    CommandDef("reasoning", "Управление reasoning effort и его отображением", "Configuration",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "Переключить fast mode — OpenAI Priority / Anthropic Fast (Normal/Fast)", "Configuration",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "Показать или сменить тему оформления", "Configuration",
               cli_only=True, args_hint="[name]"),
    CommandDef("indicator", "Выбрать стиль индикатора занятости в TUI", "Configuration",
               cli_only=True, args_hint="[kaomoji|emoji|unicode|ascii]",
               subcommands=("kaomoji", "emoji", "unicode", "ascii")),
    CommandDef("voice", "Переключить голосовой режим", "Configuration",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "Управление поведением Enter пока Hermes работает", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "Управление инструментами: /tools [list|disable|enable] [name...]", "Tools & Skills",
               args_hint="[list|disable|enable] [name...]", cli_only=True),
    CommandDef("toolsets", "Список доступных наборов инструментов", "Tools & Skills",
               cli_only=True),
    CommandDef("skills", "Поиск, установка, просмотр и управление скиллами",
               "Tools & Skills", cli_only=True,
               subcommands=("search", "browse", "inspect", "install")),
    CommandDef("cron", "Управление запланированными задачами", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "Фоновое обслуживание скиллов (статус, запуск, pin, архив)",
               "Tools & Skills", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore")),
    CommandDef("kanban", "Канбан-доска для нескольких профилей (задачи, ссылки, комментарии)",
               "Tools & Skills", args_hint="[subcommand]",
    ...
    CommandDef("reload", "Перезагрузить переменные .env в текущую сессию", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.hermes/skills/ for newly installed or removed skills",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "Подключить браузерные инструменты к live Chrome через CDP", "Tools & Skills",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "Список установленных плагинов и их статус",
               "Tools & Skills", cli_only=True),

    # Info
    CommandDef("commands", "Просмотр всех команд и скиллов (постранично)", "Info",
               gateway_only=True, args_hint="[page]"),
    CommandDef("help", "Показать доступные команды", "Info"),
    CommandDef("restart", "Корректно перезапустить gateway после слива активных задач", "Session",
               gateway_only=True),
    CommandDef("usage", "Показать использование токенов и rate-limits для сессии", "Info"),
    CommandDef("insights", "Показать аналитику использования", "Info",
               args_hint="[days]"),
    CommandDef("platforms", "Показать статус gateway и платформ обмена сообщениями", "Info",
               cli_only=True, aliases=("gateway",)),
    CommandDef("copy", "Скопировать последний ответ ассистента в буфер обмена", "Info",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "Прикрепить изображение из буфера обмена", "Info",
               cli_only=True),
    CommandDef("image", "Прикрепить локальный файл изображения к следующему промпту", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "Обновить Hermes Agent до последней версии", "Info",
               gateway_only=True),
    CommandDef("debug", "Загрузить отладочный отчёт (система + логи) и получить ссылки", "Info"),

    # Exit
    CommandDef("quit", "Выход из CLI", "Exit",
...
def gateway_help_lines() -> list[str]:
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        alias_note = f" (псевдоним: {', '.join(alias_parts)})" if alias_parts else ""
